...
from ..database import identity_context, tenant_context, is_superuser_context
...
async def identity_middleware(request: Request, call_next):
    ...
    is_superuser_context_token = None
    ...
    if auth_header.startswith("Bearer "):
        try:
            ...
            if payload and payload.get("sub") and payload.get("type") == "access":
                ...
                user_id_context_token = identity_context.set(uid)
                is_superuser_context_token = is_superuser_context.set(is_super)
                
                logger.info(f" [Identity-Gateway] Token validated for UUID: {uid} | Superuser: {is_super}")
            else:
    ...
    try:
        ...
        if is_superuser_context_token:
            is_superuser_context.reset(is_superuser_context_token)
            
        if not user_id_context_token:
            identity_context.set(None)
            is_superuser_context.set(False)
